        ...
        <div class="bg-white w-96  overflow-auto shadow-sm sm:rounded-lg">
            <div class="p-6 bg-white border-b border-gray-200">
                ...
                <form method="POST" action="{{ route('users.store') }}" enctype="multipart/form-data">
                    ...
                    @csrf
                    <div class="mb-5">
                        <p class="font-medium text-xl text-left">New User</p>
                    ...
                    <div>
                        <div class="mt-4">
                            <div class="relative">
                                <x-floating-input type="text" id="first_name" name="first_name"
                                    wire:model="first_name" class="block w-full border-2 " required />
                        ...
                        <div class="mt-4">
                            <div class="relative">
                                <x-floating-input type="email" id="email" name="email" wire:model="email"
                                    required />
                        ...
                        dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer">
                                ...
                                @foreach ($roles as $role)
                                    <option value="{{ $role->name }}"
                                        @if ($role === $role->name) selected @endif> {{ $role->name }}</option>
                                @endforeach
                            </select>
                            @error('role')
                                <p id="outlined_error_help" class="mt-2 text-left text-xs text-red-600 dark:text-red-400">
